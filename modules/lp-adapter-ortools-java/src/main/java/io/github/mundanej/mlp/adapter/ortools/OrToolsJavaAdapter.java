...
public final class OrToolsJavaAdapter implements LpSolverAdapter {
    ...
    public OrToolsJavaAdapter() {
        this(Loader::loadNativeLibraries);
    }
    ...
    private static MPVariable[] variables(
        ...
        for (int index = 0; index < variables.length; index++) {
            ...
            variables[index] = solver.makeNumVar(bound(bounds.lower()), bound(bounds.upper()), columnNames[index]);
        }
        return variables;
    }
    ...
    private static void constraints(
        ...
        for (int row = 0; row < input.matrix().rows(); row++) {
            ...
            MPConstraint constraint = solver.makeConstraint(
                    bound(bounds.lower()),
                    bound(bounds.upper()),
                    input.rowNames().get(row));
            for (int offset = rowPointers[row]; offset < rowPointers[row + 1]; offset++) {
                constraint.setCoefficient(variables[columnIndices[offset]], finite(values[offset], "matrix value"));
            }
        }
    }
    ...
    private static void objective(
        ...
        for (int index = 0; index < coefficients.length; index++) {
            objective.setCoefficient(variables[index], finite(coefficients[index], "objective coefficient"));
        }
    ...
    private static OptionalDouble objectiveValue(final SolverStatus status, final MPObjective objective) {
