...
public final class OjAlgoAdapter implements LpSolverAdapter {
  ...
  public SolverRunResult solve(
    ...
    try {
      ...
      Optimisation.Result result =
          input.problem().objective().sense() == ObjectiveSense.MAXIMIZE
              ? model.maximise()
              : model.minimise();
  ...
  private static Variable[] variables(
      final LpProblem problem, final SolverInput input, final ExpressionsBasedModel model) {
    ...
    for (int index = 0; index < variables.length; index++) {
      ...
      Variable variable =
          model
              .addVariable(input.columnNames().get(index))
  ...
  private static OptionalDouble objectiveValue(
      final LpProblem problem, final Optimisation.Result result) {
