...
public final class LpSolutionValidator {
  ...
  private static void checkStatus(
    ...
    if (evidence.status().isEmpty()) {
      findings.add(new ValidationFinding("STATUS_MISSING", "normalized status is missing", 1.0d));
      return;
    }
    if (expected.status().get() != evidence.status().get()) {
      findings.add(
          new ValidationFinding(
              "STATUS_MISMATCH", "normalized status does not match expected status", 1.0d));
    }
  }
  ...
  private static void checkRowActivity(
    ...
    for (int index = 0; index < activities.length; index++) {
      ...
      addIfViolation(
          ...
          bounds.lower() - activities[index],
          profile);
      addIfViolation(
          findings,
          "ROW_UPPER_BOUND",
          "row " + index + " violates upper bound",
          activities[index] - bounds.upper(),
          profile);
    }
  }

  private static void checkFinitePrimal(
      final double[] primal, final List<ValidationFinding> findings) {
    for (int index = 0; index < primal.length; index++) {
  ...
  private static void checkFiniteObjective(
      final ValidationEvidence evidence, final List<ValidationFinding> findings) {
  ...
  private static void checkPrimalObjective(
    ...
    if (expected.objectiveValue().isPresent()) {
      addIfViolation(
          ...
          Math.abs(evaluated - expected.objectiveValue().getAsDouble()),
          profile);
    }
    if (evidence.objectiveValue().isPresent()) {
      addIfViolation(
          ...
          Math.abs(evaluated - evidence.objectiveValue().getAsDouble()),
          profile);
    }
  }
