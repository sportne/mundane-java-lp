...
- Implement HiGHS CLI invocation through the solver SPI.
...
- Do not fail unrelated quality gates when HiGHS is not installed.

## Required tests

- Unit tests for command construction and status parsing.
- Tests for unavailable binary behavior.
- Integration smoke test guarded so it skips cleanly when HiGHS is absent.

## Required docs

- Update HiGHS adapter README and solver comparison docs.
